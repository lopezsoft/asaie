...
class Representative_report_m extends SME_Model {
	function __construct() {
		parent::__construct();
		$this->load->model('M_jreport');
	}

	public function getResults($formato,$report) {
		...
		$parm	= array(
			'P_YEAR' 	=> $this->get_year(),
		);
	
		$request	= $this->M_jreport->get_report_export($report,$report_export,$format,'',$parm);
	...
	public function getCertificate ($formato,$grade_id,$headquarter_id,$report) {
		$db			= $this->get_db_name();
		if($report == '1'){
			//Reporte a Procesar : Este nombre es del reporte creado en JasReport
		...
		$query	= "SELECT tm.*, CONCAT(RTRIM(ti.apellido1),' ',RTRIM(ti.apellido2),' ',
				...
				WHERE tm.year	= {$this->get_year()} AND tm.id_grade = {$grade_id} AND tm.id_headquarters = {$headquarter_id}
